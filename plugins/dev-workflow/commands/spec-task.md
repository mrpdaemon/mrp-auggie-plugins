...
If neither `{research_report}` nor `{design_spec}` is available, you will perform your own codebase exploration as needed during the spec process. Use `codebase-retrieval` and `mrp-explorer` agents to explore the codebase to inform the implementation spec.
...
Based on `{task_description}`, `{research_report}` (if available), and `{design_spec}` (if available), identify every component, file, and module that needs to be created or modified. Use `codebase-retrieval` and `mrp-explorer` agents to:
