...
If neither `{research_report}` nor `{design_spec}` is available, you will perform your own codebase exploration as needed during the spec process. Do quick lookups yourself, and launch `mrp-explorer` sub-agents (check your available tools for the one ending in `mrp-explorer`) for deeper exploration to inform the implementation spec.
...
Based on `{task_description}`, `{research_report}` (if available), and `{design_spec}` (if available), identify every component, file, and module that needs to be created or modified. Do quick lookups yourself, and launch `mrp-explorer` sub-agents (check your available tools for the one ending in `mrp-explorer`) to:
