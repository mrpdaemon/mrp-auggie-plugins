...
Based on `{task_description}`, `{research_report}` (if available), and `{design_spec}` (if available), investigate how the changes introduced by this task can best be tested. Use `codebase-retrieval` and `mrp-explorer` (or `sub-agent-mrp-explorer`, whichever is available) agents to:
