...
If no research report was available, use `codebase-retrieval` for quick lookups, and launch `mrp-explorer` sub-agents (check your available tools for the one ending in `mrp-explorer`) to explore the codebase as needed to inform the design questions.
