...
Launch **multiple `mrp-explorer` sub-agents in parallel**. The exact tool name varies by platform — check your available tools for the one ending in `mrp-explorer` (e.g. it may be called `mrp-explorer`, `sub-agent-dev-workflow-mrp-explorer`, or similar). Each agent should focus on a different angle of the investigation. Decide which angles to explore based on the task description, but consider the following:
...
## Step 4: Clean up explorer artifacts

Explorer sub-agents may have created `.md` files in the workspace as side effects. Find and delete any `.md` files that were generated by the explorer agents during the investigation (do **not** delete pre-existing files or the research report itself).
