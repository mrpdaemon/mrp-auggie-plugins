...
Launch **multiple `mrp-explorer` (or `sub-agent-mrp-explorer`, whichever is available) agents in parallel**. Each agent should focus on a different angle of the investigation. Decide which angles to explore based on the task description, but consider the following:
...
Write the synthesized report to `{task_dir}/{research_report}`.
