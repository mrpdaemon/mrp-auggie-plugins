...
Do **not** refer to specific tools by name (e.g., `ask-user`, `codebase-retrieval`, `web-search`). Tool names vary across runtimes (Claude Code, Augment, etc.), so naming them couples commands to a particular host. Instead, instruct the agent in terms of the action or outcome — "ask the user", "search the codebase", "look up information online" — and let the agent pick the appropriate tool from whatever it has available. Sub-agent names defined by this plugin (e.g., `mrp-explorer`, `mrp-builder`, `mrp-reviewer`) and skill names defined by this plugin (e.g., `mrp-dev-task`, `mrp-commit-message`) are part of the plugin's own surface area and may be referenced directly.
